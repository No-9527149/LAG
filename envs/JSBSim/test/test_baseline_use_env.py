...
LastEditTime : 2024-11-11 18:09:47
FilePath     : /LAG/envs/JSBSim/test/test_baseline_use_env.py
Description  : 
"""
...
class PursueAgent(BaselineAgent):
    ...
    def set_delta_value(self, env, task):
        ...
        ego_uid, enm_uid = (
            list(env.agents.keys())[self.agent_id],
            list(env.agents.keys())[(self.agent_id + 1) % 2],
        )
        ...
        # delta velocity
        delta_velocity = env.agents[enm_uid].get_property_value(
            c.velocities_u_mps
...
class ManeuverAgent(BaselineAgent):
    ...
    def set_delta_value(self, env, task):
        step_list = (
            np.arange(1, len(self.target_heading_list) + 1)
            * self.turn_interval
            / env.time_interval
        )
        uid = list(env.agents.keys())[self.agent_id]
        ...
        if (
            not self.dodge_missile
            or task._check_missile_warning(env, self.agent_id) is not None
        ):
            ...
            delta_altitude = self.target_altitude_list[i] - env.agents[
                uid
            ].get_property_value(c.position_h_sl_m)
            delta_velocity = self.target_velocity_list[i] - env.agents[
                uid
            ].get_property_value(c.velocities_u_mps)
            self.step += 1
        else:
            delta_heading = self.init_heading - cur_heading
            delta_altitude = 6000 - env.agents[uid].get_property_value(
                c.position_h_sl_m
            )
            delta_velocity = 243 - env.agents[uid].get_property_value(
                c.velocities_u_mps
            )
