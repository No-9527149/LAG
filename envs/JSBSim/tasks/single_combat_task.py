...
class SingleCombatTask(BaseTask):
    ...
    def load_variables(self):
        self.state_var = [
            c.position_long_gc_deg,  # 0. longitude  (unit: °)
    ...
    def load_observation_space(self):
        self.observation_space = spaces.Box(low=-10, high=10.0, shape=(15,))
    ...
    def get_obs(self, env, agent_id):
        """Convert simulation states into the format of observation space
        ...
        ego_AO, ego_TA, R, side_flag = get2d_AO_TA_R(
        ...
        norm_obs = np.clip(
            norm_obs, self.observation_space.low, self.observation_space.high
        )
        return norm_obs
                ...
                for enm in env.agents[agent_id].enemies:
                    if enm.is_alive:
                        enm_feature = np.hstack(
                            [enm.get_position(), enm.get_velocity()]
                        )
...
class HierarchicalSingleCombatTask(SingleCombatTask):
    ...
    def __init__(self, config: str):
        super().__init__(config)
        self.low_level_policy = BaselineActor()
        self.low_level_policy.load_state_dict(
            torch.load(
        ...
        self.norm_delta_heading = np.array(
            [-np.pi / 6, -np.pi / 12, 0, np.pi / 12, np.pi / 6]
        )
...
class StraightFlyAgent:
    def normalize_action(self, action):
        norm_act = np.zeros(4)
        norm_act[0] = action[0] / 20 - 1.0  # 0~40 => -1~1
        norm_act[1] = action[1] / 20 - 1.0  # 0~40 => -1~1
        norm_act[2] = action[2] / 20 - 1.0  # 0~40 => -1~1
...
class PursueAgent(BaselineAgent):
    ...
    def set_delta_value(self, sim: AircraftSimulator):
        ...
        # delta velocity
        delta_velocity = sim.enemies[0].get_property_value(
            c.velocities_u_mps
...
class ManeuverAgent(BaselineAgent):
    ...
    def set_delta_value(self, sim: AircraftSimulator):
        step_list = (
            np.arange(1, len(self.target_heading_list) + 1) * self.turn_interval / 0.2
        )
        ...
        if not self.dodge_missile or len(sim.under_missiles) != 0:
            for i, interval in enumerate(step_list):
                if self.step <= interval:
                    break
            delta_heading = (
                self.init_heading + self.target_heading_list[i] - cur_heading
            )
            delta_altitude = self.target_altitude_list[i] - sim.get_property_value(
                c.position_h_sl_m
            )
            delta_velocity = self.target_velocity_list[i] - sim.get_property_value(
                c.velocities_u_mps
            )
            self.step += 1
        else:
...
class DodgeMissileAgent:
    def __init__(self) -> None:
        ...
        self.actor = BaselineActor(input_dim=21, use_mlp_act_layer=True)
        self.actor.load_state_dict(
            torch.load(
                self.model_path, map_location=torch.device("cpu"), weights_only=True
            )
