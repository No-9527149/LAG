...
class MultipleCombatTask(SingleCombatTask):
    ...
    def load_observation_space(self):
        ...
        self.observation_space = spaces.Box(
            low=-10, high=10.0, shape=(self.obs_length,)
        )
        self.share_observation_space = spaces.Box(
            low=-10, high=10.0, shape=(self.num_agents * self.obs_length,)
        )
    ...
    def get_obs(self, env, agent_id):
        ...
        ego_cur_ned = LLA2NEU(
            *ego_state[:3], env.center_lon, env.center_lat, env.center_alt
        )
...
class HierarchicalMultipleCombatShootTask(HierarchicalMultipleCombatTask):
    ...
    def get_obs(self, env, agent_id):
        ...
        # (3) missile info TODO: multiple missile and partner's missile?
        ...
        if missile_sim is not None:
            ...
            norm_obs[offset + 1] = (
                np.linalg.norm(missile_sim.get_velocity()) - ego_state[9]
            ) / 340
    ...
    def step(self, env):
        ...
        for agent_id, agent in env.agents.items():
            ...
            target_list = list(
                map(lambda x: x.get_position() - agent.get_position(), agent.enemies)
            )
            ...
            shoot_flag = (
                ...
                and self._remaining_missiles[agent_id] > 0
            ...
            if shoot_flag:
                ...
                env.add_temp_simulator(
                    MissileSimulator.create(
                        parent=agent,
                        target=agent.enemies[target_index],
                        uid=new_missile_uid,
                    )
                )
