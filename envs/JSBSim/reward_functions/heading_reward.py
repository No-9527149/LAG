...
LastEditTime : 2024-11-11 18:05:57
FilePath     : /LAG/envs/JSBSim/reward_functions/heading_reward.py
Description  : 
"""
...
class HeadingReward(BaseRewardFunction):
    ...
    def get_reward(self, task, env, agent_id, info={}):
        ...
        heading_r = math.exp(
            -(
                (
                    env.agents[agent_id].get_property_value(c.delta_heading)
                    / heading_error_scale
                )
                ** 2
            )
        ...
        alt_r = math.exp(
            -(
                (
                    env.agents[agent_id].get_property_value(c.delta_altitude)
                    / alt_error_scale
                )
                ** 2
            )
        ...
        roll_r = math.exp(
            -(
                (
                    env.agents[agent_id].get_property_value(c.attitude_roll_rad)
                    / roll_error_scale
                )
                ** 2
            )
        ...
        speed_r = math.exp(
            -(
                (
                    env.agents[agent_id].get_property_value(c.delta_velocities_u)
                    / speed_error_scale
                )
                ** 2
            )
