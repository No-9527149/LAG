...
LastEditTime : 2024-11-11 18:20:46
FilePath     : /LAG/algorithms/utils/buffer.py
Description  : 
"""
...
class ReplayBuffer(Buffer):
    ...
    def __init__(self, args, num_agents, obs_space, act_space):
        ...
        self.obs = np.zeros(
            (self.buffer_size + 1, self.n_rollout_threads, self.num_agents, *obs_shape),
            dtype=np.float32,
        )
        self.actions = np.zeros(
            (self.buffer_size, self.n_rollout_threads, self.num_agents, *act_shape),
            dtype=np.float32,
        )
        ...
        self.masks = np.ones(
            (self.buffer_size + 1, self.n_rollout_threads, self.num_agents, 1),
            dtype=np.float32,
        )
        ...
        # V(o), R(o) while advantage = returns - value_predictions
        self.value_predictions = np.zeros(
            (self.buffer_size + 1, self.n_rollout_threads, self.num_agents, 1),
            dtype=np.float32,
        )
    ...
    def advantages(self) -> np.ndarray:
        advantages = self.returns[:-1] - self.value_predictions[:-1]  # type: np.ndarray
    ...
    def insert(
        ...
        **kwargs
        ...
        self.value_predictions[self.step] = value_predictions.copy()
    ...
    def clear(self):
        ...
        self.value_predictions = np.zeros_like(self.value_predictions, dtype=np.float32)
    ...
    def recurrent_generator(
        buffer: Union[Buffer, List[Buffer]], num_mini_batch: int, data_chunk_length: int
    ):
        """
        ...
        Args:
            ...
            num_mini_batch (int): number of mini batches to split the batch into.
        ...
        Returns:
            (obs_batch, actions_batch, masks_batch, old_action_log_probs_batch, advantages_batch, \
                returns_batch, value_predictions_batch, rnn_states_actor_batch, rnn_states_critic_batch)
        """
        buffer = (
            [buffer] if isinstance(buffer, ReplayBuffer) else buffer
        )  # type: List[ReplayBuffer]
        n_rollout_threads = buffer[0].n_rollout_threads
        buffer_size = buffer[0].buffer_size
        num_agents = buffer[0].num_agents
        assert (
            all([b.n_rollout_threads == n_rollout_threads for b in buffer])
        ...
        assert n_rollout_threads * buffer_size >= data_chunk_length, (
            ...
            "data chunk length ({}).".format(
                n_rollout_threads, buffer_size, num_agents, data_chunk_length
            )
        )
        ...
        old_action_log_probs = np.vstack(
            [ReplayBuffer._cast(buf.action_log_probs) for buf in buffer]
        )
        ...
        value_predictions = np.vstack(
            [ReplayBuffer._cast(buf.value_predictions[:-1]) for buf in buffer]
        )
        rnn_states_actor = np.vstack(
            [ReplayBuffer._cast(buf.rnn_states_actor[:-1]) for buf in buffer]
        )
        rnn_states_critic = np.vstack(
            [ReplayBuffer._cast(buf.rnn_states_critic[:-1]) for buf in buffer]
        )
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                old_action_log_probs_batch.append(
                    old_action_log_probs[ind : ind + data_chunk_length]
                )
                advantages_batch.append(advantages[ind : ind + data_chunk_length])
                returns_batch.append(returns[ind : ind + data_chunk_length])
                value_predictions_batch.append(
                    value_predictions[ind : ind + data_chunk_length]
                )
            ...
            # These are all from numpy of size (L, N, Dim)
            ...
            value_predictions_batch = np.stack(value_predictions_batch, axis=1)

            # States is just a (N, -1) from_numpy
            rnn_states_actor_batch = np.stack(rnn_states_actor_batch).reshape(
                N, *buffer[0].rnn_states_actor.shape[3:]
            )
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(
                N, *buffer[0].rnn_states_critic.shape[3:]
            )

            # Flatten the (L, N, ...) from numpy to (L * N, ...)
            ...
            old_action_log_probs_batch = ReplayBuffer._flatten(
            ...
            value_predictions_batch = ReplayBuffer._flatten(
                L, N, value_predictions_batch
            )

            yield obs_batch, actions_batch, masks_batch, old_action_log_probs_batch, advantages_batch, returns_batch, value_predictions_batch, rnn_states_actor_batch, rnn_states_critic_batch
    ...
    def recurrent_generator(
        ...
        Returns:
            (obs_batch, share_obs_batch, actions_batch, masks_batch, active_masks_batch, \
                old_action_log_probs_batch, advantages_batch, returns_batch, value_predictions_batch, \
        ...
        value_predictions = self._cast(self.value_predictions[:-1])
        ...
        for indices in sampler:
            ...
            value_predictions_batch = self._flatten(L, N, value_predictions_batch)

            yield obs_batch, share_obs_batch, actions_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, advantages_batch, returns_batch, value_predictions_batch, rnn_states_actor_batch, rnn_states_critic_batch
