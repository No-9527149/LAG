...
LastEditTime : 2024-11-11 18:22:18
FilePath     : /LAG/algorithms/utils/gru.py
Description  : 
"""
...
class GRULayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, hxs: torch.Tensor, masks: torch.Tensor):
        ...
        if x.size(0) == hxs.size(0):
            ...
            x, hxs = self.gru(
                x.unsqueeze(0),
                (hxs * masks.repeat(1, self._num_layers).unsqueeze(-1))
                .transpose(0, 1)
                .contiguous(),
            )
        ...
        # (T>1): x=[T * N, input_size], hxs=[N, L, hidden_size], masks=[T * N, 1]
            ...
            for i in range(len(has_zeros) - 1):
                ...
                temp = (
                    hxs * masks[start_idx].view(1, -1, 1).repeat(self._num_layers, 1, 1)
                ).contiguous()
