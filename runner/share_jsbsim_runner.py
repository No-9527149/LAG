...
class ShareJSBSimRunner(Runner):
    ...
    def eval(self, total_num_steps):
        ...
        if self.use_selfplay:
            ...
            assert (
                self.eval_episodes >= self.all_args.n_choose_opponents
        ...
        while total_episodes < self.eval_episodes:
            ...
            if (
                ...
                logging.info(
                    f" Load opponent {policy_idx} for evaluation ({total_episodes+1}/{self.eval_episodes})"
                )
                ...
                eval_opponent_rnn_states = np.zeros_like(
                    eval_rnn_states, dtype=np.float32
                )
            ...
            if self.use_selfplay:
                ...
                eval_opponent_rnn_states = np.array(
                    np.split(
                        _t2n(eval_opponent_rnn_states), self.n_eval_rollout_threads
                    )
                )
                eval_opponent_actions = np.array(
                    np.split(_t2n(eval_opponent_actions), self.n_eval_rollout_threads)
                )
                eval_actions = np.concatenate(
                    (eval_actions, eval_opponent_actions), axis=1
                )
            ...
            if self.use_selfplay:
                eval_opponent_obs = eval_obs[:, self.num_agents // 2 :, ...]
                eval_obs = eval_obs[:, : self.num_agents // 2, ...]
                eval_opponent_masks[eval_dones_env == True] = np.zeros(
                    ((eval_dones_env == True).sum(), *eval_opponent_masks.shape[1:]),
                    dtype=np.float32,
                )
                eval_opponent_rnn_states[eval_dones_env == True] = np.zeros(
                    (
                        (eval_dones_env == True).sum(),
                        *eval_opponent_rnn_states.shape[1:],
                    ),
                    dtype=np.float32,
                )
    ...
    def render(self):
        ...
        if self.use_selfplay:
            ...
            self.eval_opponent_policy.actor.load_state_dict(
                torch.load(
                    str(self.model_dir) + f"/actor_{policy_idx}.pt", weights_only=True
                )
            )
            self.eval_opponent_policy.prep_rollout()
            # reset obs/rnn/mask
        ...
        logging.info(
            "render episode reward of agent: "
            + str(render_infos["render_episode_reward"])
        )
    ...
    def save(self, episode):
        ...
        if self.use_selfplay:
            torch.save(
                policy_actor_state_dict, str(self.save_dir) + f"/actor_{episode}.pt"
            )
    ...
    def reset_opponent(self):
        ...
        for policy in self.opponent_policy:
            ...
            policy.actor.load_state_dict(
                torch.load(str(self.save_dir) + f"/actor_{choose_idx}.pt")
            )
            policy.prep_rollout()
