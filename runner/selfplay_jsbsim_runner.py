...
LastEditTime : 2024-11-27 16:05:31
...
class SelfplayJSBSimRunner(JSBSimRunner):
    ...
    def load(self):
        ...
        self.buffer = ReplayBuffer(
            self.all_args, self.num_agents // 2, self.obs_space, self.act_space
        )
        ...
        self.opponent_env_split = np.array_split(
            np.arange(self.n_rollout_threads), len(self.opponent_policy)
        )
    ...
    def insert(self, data: List[np.ndarray]):
        ...
        self.opponent_rnn_states[dones_env == True] = np.zeros(
            ((dones_env == True).sum(), *rnn_states_actor.shape[1:]), dtype=np.float32
        )
    ...
    def eval(self, total_num_steps):
        ...
        while total_episodes < self.eval_episodes:
            ...
            if total_episodes >= eval_cur_opponent_idx * eval_each_episodes:
                ...
                self.eval_opponent_policy.actor.load_state_dict(
                    torch.load(
                        str(self.save_dir) + f"/actor_{policy_idx}.pt",
                        weights_only=True,
                    )
                )
                ...
                rnn_states = np.zeros(
                    (
                        ...
                        *self.buffer.rnn_states_actor.shape[2:],
                    ),
                    dtype=np.float32,
                )
            ...
            rnn_states = np.array(
                np.split(_t2n(rnn_states), self.n_eval_rollout_threads)
            )
            ...
            opponent_rnn_states = np.array(
                np.split(_t2n(opponent_rnn_states), self.n_eval_rollout_threads)
            )
            opponent_actions = np.array(
                np.split(_t2n(opponent_actions), self.n_eval_rollout_threads)
            )
            actions = np.concatenate((actions, opponent_actions), axis=1)
            ...
            obs = obs[:, : self.num_agents // 2, ...]
            masks[dones_env == True] = np.zeros(
                ((dones_env == True).sum(), *masks.shape[1:]), dtype=np.float32
            )
            rnn_states[dones_env == True] = np.zeros(
                ((dones_env == True).sum(), *rnn_states.shape[1:]), dtype=np.float32
            )

            opponent_masks[dones_env == True] = np.zeros(
                ((dones_env == True).sum(), *opponent_masks.shape[1:]), dtype=np.float32
            )
            opponent_rnn_states[dones_env == True] = np.zeros(
                ((dones_env == True).sum(), *opponent_rnn_states.shape[1:]),
                dtype=np.float32,
            )
            ...
            opponent_episode_rewards.append(
                opponent_cumulative_rewards[dones_env == True]
            )
        ...
        episode_rewards = np.concatenate(
            episode_rewards
        )  # shape (self.eval_episodes, self.num_agents, 1)
        episode_rewards = episode_rewards.squeeze(-1).mean(
            axis=-1
        )  # shape: (self.eval_episodes,)
        eval_average_episode_rewards = np.array(
            np.split(episode_rewards, self.num_opponents)
        ).mean(
            axis=-1
        )  # shape (self.num_opponents,)
        ...
        opponent_average_episode_rewards = np.array(
            np.split(opponent_episode_rewards, self.num_opponents)
        ).mean(axis=-1)

        # Update elo
        ego_elo = np.array(
            [self.latest_elo for _ in range(self.n_eval_rollout_threads)]
        )
        opponent_elo = np.array(
            [self.policy_pool[key] for key in eval_choose_opponents]
        )
    ...
    def reset_opponent(self):
        ...
        for policy in self.opponent_policy:
            ...
            policy.actor.load_state_dict(
                torch.load(
                    str(self.save_dir) + f"/actor_{choose_idx}.pt", weights_only=True
                )
            )
            policy.prep_rollout()
        logging.info(
    ...
    def render(self):
        ...
        self.policy.actor.load_state_dict(
            torch.load(str(self.model_dir) + f"/actor_{idx}.pt")
        )
        self.policy.prep_rollout()
        self.eval_opponent_policy.actor.load_state_dict(
            torch.load(
                str(self.model_dir) + f"/actor_{opponent_idx}.pt", weights_only=True
            )
        )
        self.eval_opponent_policy.prep_rollout()
        logging.info(
        ...
        self.envs.render(
            mode="txt", filepath=f"{file_path}/{self.experiment_name}.txt.acmi"
        )
