...
LastEditTime : 2024-11-11 18:14:49
FilePath     : /LAG/test_env.py
Description  : 
"""
...
def test_env():
    ...
    envs = DummyVecEnv(
        [
            lambda: SingleCombatEnv("1v1/NoWeapon/HierarchySelfplay")
    ...
    while True:
        ...
        bloods = [
            envs.envs[0].agents[agent_id].bloods
            for agent_id in envs.envs[0].agents.keys()
        ]
...
def test_multi_env():
    parallel_num = 1
    envs = ShareDummyVecEnv(
        [
            lambda: MultipleCombatEnv("2v2/NoWeapon/HierarchySelfplay")
            for _ in range(parallel_num)
        ]
    )
