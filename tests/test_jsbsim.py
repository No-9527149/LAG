...
LastEditTime : 2024-11-11 18:13:43
FilePath     : /LAG/tests/test_jsbsim.py
Description  : 
"""
...
from envs.JSBSim.envs.single_control_env import SingleControlEnv
from envs.JSBSim.envs.single_combat_env import SingleCombatEnv
from envs.JSBSim.envs.multiple_combat_env import MultipleCombatEnv
...
class TestSingleControlEnv:
    ...
    def test_env(self):
        ...
        while True:
            ...
            assert (
                obs.shape == obs_shape
                and actions.shape == act_shape
                and reward.shape == reward_shape
                and done.shape == done_shape
            )
        ...
        while t < len(done_buff):
            ...
            assert (
                np.linalg.norm(obs - obs_buf[t + 1]) < 1e-8
                and np.all(reward == rew_buf[t])
                and np.all(done == done_buff[t])
            )
            t += 1

    @pytest.mark.parametrize("vec_env", [DummyVecEnv, SubprocVecEnv])
    def test_vec_env(self, vec_env):
        parallel_num = 4
        envs = vec_env(
            [lambda: SingleControlEnv("1/heading") for _ in range(parallel_num)]
        )
        ...
        assert observation.shape == obs_shape
        ...
        while True:
            ...
            assert (
                ...
                and actions.shape == act_shape
                and rewards.shape == reward_shape
                and dones.shape == done_shape
...
class TestSingleCombatEnv:
    ...
    @pytest.mark.parametrize(
        "config",
        [
            "1v1/NoWeapon/vsBaseline",
            "1v1/NoWeapon/Selfplay",
            "1v1/DodgeMissile/vsBaseline",
            "1v1/DodgeMissile/Selfplay",
            "1v1/DodgeMissile/HierarchyVsBaseline",
            "1v1/DodgeMissile/HierarchySelfplay",
        ],
    )
    def test_env(self, config):
        ...
        while True:
            ...
            if env.current_step % 2 == 0:
                ...
                actions = np.array(
                    [action_space.sample() for _ in range(env.num_agents)]
                )
        ...
        while t < len(done_buff):
            ...
            assert (
                np.linalg.norm(obs - obs_buf[t + 1]) < 1e-8
                and np.all(rewards == rew_buf[t])
                and np.all(dones == done_buff[t])
            )
            t += 1

    @pytest.mark.parametrize(
        "config", ["1v1/NoWeapon/vsBaseline", "1v1/NoWeapon/Selfplay"]
    )
    ...
    @pytest.mark.parametrize(
        "config", ["1v1/DodgeMissile/vsBaseline", "1v1/DodgeMissile/Selfplay"]
    )
    def test_agent_shot_down(self, config):
        # if has weapon, once enemy die, env terminate until no missile warning!
        env = SingleCombatEnv(config)
        env.seed(0)
        obs = env.reset()
        crash_id = env.ego_ids[0]  # ego shot_down
        while True:
            # manual crash
            if env.current_step == 1:
                ...
                env.add_temp_simulator(
                    MissileSimulator.create(
                        env.agents[crash_id], env.agents[crash_id].enemies[0], "C0000"
                    )
                )
                env.agents[crash_id].shot_down()
                crash_obs = obs[0]
            actions = np.array(
            ...
            if np.all(dones):
                ...
                assert (
                    ...
                    and any(
                        [
                            missile.is_alive
                            for missile in env.agents[crash_id].launch_missiles
                        ]
                    )
                )
    ...
    @pytest.mark.parametrize(
        ...
        list(
            product(
                ...
                ["1v1/DodgeMissile/Selfplay", "1v1/DodgeMissile/HierarchyVsBaseline"],
            )
        ),
    )
    def test_vec_env(self, vec_env, config):
        parallel_num = 4
        envs = vec_env([lambda: SingleCombatEnv(config) for _ in range(parallel_num)])
...
class TestJSBSimRunner:
    ...
    def test_training(self, args):
        ...
        args += (
            " --experiment-name pytest --seed 1 --n-training-threads 1 --n-rollout-threads 5 --cuda"
...
class TestMultipleCombatEnv:
    ...
    def test_env(self, config):
        ...
        t = 0
        assert (
            np.linalg.norm(obs - obs_buf[t]) < 1e-8
    ...
    def test_agent_die(self):
        ...
        while True:
            ...
            if env.current_step == 40:
                ...
                env.add_temp_simulator(
                    MissileSimulator.create(
                        env.agents[enemy1_id], env.agents[uid], uid="C0000"
                    )
                )
    ...
    def test_vec_env(self, vec_env, config):
        parallel_num = 4
        envs = vec_env([lambda: MultipleCombatEnv(config) for _ in range(parallel_num)])
        assert envs.num_agents == 4
        obs_shape = (parallel_num, envs.num_agents, *envs.observation_space.shape)
        share_obs_shape = (
            parallel_num,
            envs.num_agents,
            *envs.share_observation_space.shape,
        )
        reward_shape = (parallel_num, envs.num_agents, 1)
        done_shape = (parallel_num, envs.num_agents, 1)
