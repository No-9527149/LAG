...
LastEditTime : 2024-11-11 18:13:53
FilePath     : /LAG/tests/test_ppo.py
Description  : 
"""
...
class TestPPO:
    ...
    @pytest.mark.parametrize(
        "obs_space, act_space, batch_size",
        list(
            product(
                [gym.spaces.Box(low=-1, high=1, shape=(18,))],  # obs_space
                [  # act_space
    ...
    def test_ppo_actor(self, obs_space, act_space, batch_size):
        ...
        action_log_probs, dist_entropy = actor.evaluate_actions(
            obs, init_rnn_states, pre_actions, masks
        )
        assert action_log_probs.shape == (batch_size, 1)
        assert dist_entropy.shape == (batch_size, 1)

    @pytest.mark.parametrize(
        "obs_space, batch_size",
        list(
            product(
                [gym.spaces.Box(low=-1, high=1, shape=(18,))],  # obs_space
                [1, 5],  # batch_size
    ...
    def test_ppo_critic(self, obs_space, batch_size):
        critic = PPOCritic(
            get_config().parse_args(args=""), obs_space, device=torch.device("cpu")
        )

        obs = np.array([obs_space.sample() for _ in range(batch_size)])
        masks = np.ones((batch_size, 1))
        init_rnn_states = np.zeros(
            (batch_size, critic.recurrent_hidden_layers, critic.recurrent_hidden_size)
        )
    ...
    @pytest.mark.parametrize(
        "num_agents, obs_space, act_space, num_mini_batch, data_chunk_length",
        list(
            product(
                [1, 2],  # num_agents
                [gym.spaces.Box(low=-1, high=1, shape=(18,))],  # obs_space
    ...
    def test_ppo_buffer(
        self, num_agents, obs_space, act_space, num_mini_batch, data_chunk_length
    ):
        buffer = ReplayBuffer(
            get_config().parse_args(args=""), num_agents, obs_space, act_space
        )

        obs = np.array(
            [
                [obs_space.sample() for _ in range(num_agents)]
                for _ in range(buffer.n_rollout_threads)
            ]
        )
        actions = np.array(
            [
                [act_space.sample() for _ in range(num_agents)]
                for _ in range(buffer.n_rollout_threads)
            ]
        )
        ...
        value_predictions = np.random.randn(buffer.n_rollout_threads, num_agents, 1)
        rnn_states_actor = np.zeros(
            (
                buffer.n_rollout_threads,
                num_agents,
                buffer.recurrent_hidden_layers,
                buffer.recurrent_hidden_size,
            )
        )
        ...
        batch_count = 0
        for data in buffer.recurrent_generator(
            buffer, num_mini_batch, data_chunk_length
        ):
            (
                obs_batch,
                actions_batch,
    ...
    @pytest.mark.parametrize(
        "num_agents, obs_space, act_space",
        list(
            product(
                [1, 2],  # num_agents
                [gym.spaces.Box(low=-1, high=1, shape=(18,))],  # obs_space
